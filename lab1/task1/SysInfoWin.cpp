...
namespace
{
  ...
  DWORDLONG GetPrecisionValue(const Precision p)
  {
    return static_cast<DWORDLONG>(p);
  }

  std::string PrecisionToString(const Precision precision)
  {
    switch (precision)
    {
      case Precision::KB: return "KB";
      case Precision::MB: return "MB";
      case Precision::GB: return "GB";
    }
...
void PrintRAM(const Precision precision)
{
  MEMORYSTATUSEX statex;
  statex.dwLength = sizeof(statex);
  GlobalMemoryStatusEx(&statex);

  const auto div = GetPrecisionValue(precision);
  ...
  std::cout << "RAM: " << totalRam - freeRam << " " << PrecisionToString(precision) << " / "
            << totalRam << " " << PrecisionToString(precision) << "\n";
}

void PrintVirtualMemory(const Precision precision)
{
  ...
  const double totalVRAM = static_cast<double>(pi.CommitLimit * pi.PageSize) /
    static_cast<double>(GetPrecisionValue(precision));

  std::cout << std::fixed << std::setprecision(2);
  std::cout << totalVRAM << " " << PrecisionToString(precision) << "\n";
}
...
void PrintPageFile(const Precision precision)
{
  PERFORMANCE_INFORMATION pi;
  pi.cb = sizeof(pi);
  std::cout << "Pagefile: ";
  ...
  std::cout << commitedPagefileSize / div << PrecisionToString(precision) << " / "
    << totalPagefileSize / div << PrecisionToString(precision) << "\n";
}
...
void PrintDrives(const Precision precision)
{
  std::cout << "Drives: \n";
  ...
  while (*driveName != '\0')
  {
    ...
    std::cout << std::fixed << std::setprecision(2);
    std::cout << (totalDriveSize - freeDrivePart) / div << PrecisionToString(precision) << " free / "
              << totalDriveSize / div << PrecisionToString(precision) << " total \n";
